...
Namespace Controllers.Areas
    <Authorize()>
    <HandleError>
    Public Class NegadasController
...
#Region "Constructores"
        ...
        Public H_SalesOrder As New NegadasHelper
...
#Region "Vistas"
        Public Async Function CrearNegadas() As Task(Of ActionResult)
            ...
            End If
        End Function
        Public Function ConsultarNegadas() As ActionResult
            ...
            If WebSecurity.IsAuthenticated Then
                ...
                Else
                    ...
                    Dim ValidaRespuesta As RespuestaServicios = H_Search.GetNegadasForLocation(GetEstimaciones)
                    Dim l_Busqueda As List(Of MapNegadas) = ValidaRespuesta.Valor
                    ViewBag.Negadas = l_Busqueda.OrderByDescending(Function(x) x.FechaCreacion).ToList()
                    Return View()
                End If
        ...
        Public Function ConsultarNegadasDisponiblesVer2(ByVal model As ConsultaNegadasViewModel) As JsonResult

            'Dim l_SalesOrder As New List(Of SalesOrder)
            Dim l_salesorder As New List(Of MapNegadas)
            Dim fromDate = ""
            Dim toDate = ""
            ...
            If User.IsInRole("SuperAdmin") Or User.IsInRole("CxC") Then
                ...
                Dim ValidaRespuesta As RespuestaServicios = H_Search.GetListNegadasForLocationFilters(GetSalesOrder)
        ...
        <HttpPost>
        <ValidateAntiForgeryToken()>
        Public Async Function CancelarNegada2(ByVal NS_ID As String, ByVal Folio As String) As Task(Of JsonResult)
            Try

                Dim Respuesta = H_SalesOrder.CancelNegada(NS_ID)
                ...
                Else
                    Return Json(New RespuestaControlGeneral(EnumTipoRespuesta.Fracaso, "Hubo un problema al cancelar esta negada. Detalles: " + Respuesta.Mensaje, ""))
                End If
        ...
        End Function
        <HttpPost()>
        Public Async Function GetNegadas(ByVal id As String) As Task(Of JsonResult)
            Try
                Dim ConsultaEstimacion As New NegadasMapViewModel
                Dim l_Detalle As New List(Of DetalleNegadasMapViewModel)

                Dim Estimacion As NetsuiteLibrary.SuiteTalk.Estimate = H_SalesOrder.GetEstimateByID(id)

                If IsNothing(Estimacion) Then
                    Return Json(New RespuestaControlGeneral(EnumTipoRespuesta.Fracaso, "No fue posible localizar la negada..."))
                End If

                For Each RegistrarItems As EstimateItem In Estimacion.itemList.item
                    Dim RegMercancia As New DetalleNegadasMapViewModel
            ...
            Catch ex As Exception
                Return Json(New RespuestaControlGeneral(EnumTipoRespuesta.Fracaso, "Hubo un problema al localizar la negada, contacte con el administrador..."))
            End Try
...
#Region "Procesos"
        ...
        Public Async Function ConsultarInventario(ByVal id As String, Optional ByVal NombreProducto As String = "", Optional ByVal Customer As String = "") As Task(Of JsonResult)
            ...
            Try
                ...
                Dim l_InventarioViewModel As New List(Of NegadasProductoViewModel)
                ...
                If Not IsNothing(TipoMaterial) Then
                    ...
                    If InventarioDisponible.Estatus = "Exito" Then
                        ...
                        Dim MapDatos As New NegadasProductoViewModel
        ...
        <HttpPost>
        <ValidateAntiForgeryToken()>
        Public Async Function CrearNegadas(ByVal model As NegadasViewModel, ByVal Carrito As List(Of NegadasCarritoViewModel)) As Task(Of ActionResult)

            Dim AltaOrden As New AltaNegadas
            Dim l_Detalle As New List(Of DetalleNegadas)
...
#Region "Validar Selección Mostrador y Validación de Ubicación"
                ...
                If IsNothing(UbicacionUsuario.MostradorPref) Then
                    Flash.Instance.Error("Para poder generar una negada, es necesario seleccionar un mostrador.")
                    Return View()
                End If
...
#Region "Mapeo de Campos"
                ...
                For Each RegistrarProd In Carrito
                    Dim Detalle As New DetalleNegadas
...
#Region "Validacion de Respuesta y alta en base de datos"
                If Resultado.Estatus = "Exito" Then
                    ...
                    db.Negadas.Add(RegistrarVenta)
                    db.SaveChanges()
                    Flash.Instance.Success("Se ha creado la negada con éxito! " + ValidaSO.tranId)
                    Return RedirectToAction("ConsultarNegadas")
                End If
...
#Region "Carrito"
        <HttpPost>
        <ValidateAntiForgeryToken()>
        Public Function CrearListaProductos(ByVal model As NegadasCarritoViewModel) As JsonResult
            ...
            If IsNothing(tempDataDictionary) Then
                ...
                Dim l_CarritoRender As New List(Of NegadasCarritoViewModel)
