...
namespace NetsuiteLibrary.SuiteTalk_Helpers
{
    ...
    {
        public RespuestaServicios CancelSalesOrder(string id)
        {
            try
            {
                NetSuiteService service = new NetSuiteService();
                RespuestaServicios resp = new RespuestaServicios();
                SalesOrder consulta = GetSalesOrderByID(id);


                SalesOrder salesorder = new SalesOrder();
                //salesorder.memo = "L IS REAL 2077";
                salesorder.internalId = id;

                foreach (SalesOrderItem itemSalesOrder in consulta.itemList.item)
                {

                    itemSalesOrder.isClosed = true;
                    itemSalesOrder.isClosedSpecified = true;
                }

                salesorder.itemList = consulta.itemList;

                service = ConnectionManager.GetNetSuiteService();

                WriteResponse writeResponse = service.update(salesorder);
                ...
                else
                {
                    ...
                    resp.Mensaje = string.Format("Hubo un problema al crear la orden de compra. Detalle: {0}", Error);
                    return resp;
                }
            } catch (Exception ex)
            {
        ...
        public RespuestaServicios InsertSalesOrder(AltaSalesOrder altaOrden)
        {
            try
            {
                ...
                SalesOrder SalesOrderalta = new SalesOrder();

                //Declaracion de Mercancia
                SalesOrderItemList l_salesorder = new SalesOrderItemList();
                List<SalesOrderItem> ArrayItems = new List<SalesOrderItem>();
                ...
                SalesOrderalta.subsidiary = subsidiaryRef;

                #region "Items"

                foreach (DetalleAltaSalesOrder RegMerc in altaOrden.DetalleInventario)
                {
                ...
                SalesOrderalta.memo = altaOrden.memo;
                SalesOrderalta.entity = customerRef;
                SalesOrderalta.location = LocationRef;
                SalesOrderalta.itemList = l_salesorder;
                SalesOrderalta.tranDate = DateTime.Now;
                SalesOrderalta.customFieldList = customFieldRef;
                //SalesOrderalta.currency = currencyRef;

                //Conexión NetSuite
                service = ConnectionManager.GetNetSuiteService();
                WriteResponse writeResponse = service.add(SalesOrderalta);
        ...
        public SalesOrder GetSalesOrderByID(string id)
        {
            ...
            service = ConnectionManager.GetNetSuiteService();
            salesOrder = (SalesOrder)service.get(recordRef).record;
        ...
        public RespuestaServicios TransformSalesOrderToReturnAutorization(string SalesOrder)
        {
            try
            {
                ...
                retornoMercancia.ccApproved = true;
                retornoMercancia.ccApprovedSpecified = true;
