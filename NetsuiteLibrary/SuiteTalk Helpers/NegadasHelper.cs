...
namespace NetsuiteLibrary.SuiteTalk_Helpers
{
    ...
    public class NegadasHelper
    {
        public RespuestaServicios CancelNegada(string id)
        {
            try
            {
                ...
                else
                {
                    ...
                    resp.Mensaje = string.Format("Hubo un problema al cancelar la Negada. Detalle: {0}", Error);
                    return resp;
                }
            } catch (Exception ex)
            {
                RespuestaServicios resp = new RespuestaServicios();
                resp.Estatus = "Fracaso";
                resp.Mensaje = String.Format("Hubo un Problema al cancelar la negada. Detalles: {0}.", ex.Message);
                return resp;
            }
        }

        public RespuestaServicios InsertNegada(AltaNegadas altaOrden)
        {
            try
            {
                ...
                foreach (DetalleNegadas RegMerc in altaOrden.DetalleInventario)
                {
                ...
                estatus_estimate.type = RecordType.customerStatus;
                estatus_estimate.typeSpecified = true;
                estatus_estimate.internalId = "18";
                SalesEstimacion.entityStatus = estatus_estimate;
                ...
                } else
                {
                    resp.Estatus = "Fracaso";
                    var Error = ConnectionManager.GetStatusDetails(writeResponse.status);
                    resp.Valor = string.Format("Hubo un problema al crear la negada. Detalle: {0}", Error);
                    return resp;
                }
            ...
            catch (Exception ex)
            {
                RespuestaServicios resp = new RespuestaServicios();
                resp.Estatus = "Fracaso";
                resp.Valor = String.Format("Hubo un Problema al Crear la Negada. Detalles: {0}.", ex.Message);
                return resp;
            }
